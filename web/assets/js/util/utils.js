...
class RandomUtil {
    ...
    static randomBytes(count) {
        ...
        if (typeof globalThis !== 'undefined' && globalThis.crypto && globalThis.crypto.getRandomValues) {
            ...
            for (let i = 0; i < count; ++i) {
                bytes[i] = this.randomInt(256);
            }
        }
        return bytes;
    }

    static randomBase64(count, urlSafe=false) {
        const chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/';
        const bytes = this.randomBytes(count);
        let value = '';
        for (let i = 0; i < bytes.length; i += 3) {
            ...
            value += chars[a >> 2];
            value += chars[((a & 3) << 4) | (b >> 4)];
            value += i + 1 < bytes.length ? chars[((b & 15) << 2) | (c >> 6)] : '=';
            value += i + 2 < bytes.length ? chars[c & 63] : '=';
        }
        if (urlSafe) {
    ...
    static randomShadowsocksPassword(method) {
        if (!method || !method.startsWith('2022-blake3-')) {
